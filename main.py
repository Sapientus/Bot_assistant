...
class AddressBook(UserDict):
    ...
    def save(self):
        with open('all_records.txt', 'wb') as fh:
                pickle.dump(self.data, fh)
        return 'Your data was successfully saved!'

    def load(self):
        with open('all_records.txt', 'rb') as fh:
            unpacked = pickle.load(fh)
        return f'Saved data: {unpacked}'
    
    @input_error
    def find(self, hint):             #This method prints found matches
        all_matches = []
        if hint.isalpha():
            for user in self.data:
                if hint.lower() in user.lower():
                    all_matches.append(self.data[user])
        else:
            for number in self.data.values():
                if hint in number:
                    all_matches.append(number)
        if all_matches:
            return f'All matches are: {all_matches}'
        else:
            return'There is no match, sorry :('
...
if __name__ == '__main__':
    ...
    #Here I check if our data is saved in the file
    address.save()

    #This one checks if unpacking works correctly
    print(address.load())

    #Here I check if the user can find contacts by number or name
    print(address.find('123'))
    print(address.find('j'))

    #I check its reaction when hint was not found
